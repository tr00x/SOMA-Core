...
### What is definitely not working yet

1. **stop.py pressure_trajectory** — In production, stop.py only has the final engine snapshot, not per-action pressure history. The trajectory will be `[final_pressure]` not the full `[p0, p1, ..., pN]`. To get full trajectory, post_tool_use.py would need to append each action's pressure to a session file.

2. **Real LLM impact** — The live benchmark showed SOMA detects problems (pressure rises, guidance injects) but doesn't prove the agent changes behavior. The injection reflexes produce messages, but we can't measure whether Haiku reads "[SOMA] Start implementing" and actually starts implementing.

---

*1062 tests pass. 0 mocks in verification. All measured values are from real runtime execution.*
