...
def test_cost_spiral_fires_on_errors_plus_resource_pressure() -> None:
    """cost_spiral fires on (5+ errors in last 8 actions) AND
    (token_usage>=0.5 OR budget_health<=0.4). The retry-loop-while-
    burning-tokens pattern, not pure cost growth.
    """
    cg = ContextualGuidance(profile=None)
    # 5 errors in 5 actions + low budget = textbook cost spiral.
    log = [{"tool": "Bash", "error": True, "output": f"err{i}"} for i in range(5)]
    msg = _eval(
        cg, action_log=log,
        current_tool="Bash",
        vitals={"token_usage": 0.7, "context_usage": 0.7},
        budget_health=0.2,
    )
    assert msg is not None, (
        "cost_spiral conditions met (5/5 errors + 70% context + 20% "
        "budget) but evaluate() returned None"
    )
    # cost_spiral has highest priority among the resource-aware
    # patterns when both errors and pressure are present.
    assert msg.pattern == "cost_spiral", (
        f"expected cost_spiral on textbook conditions, got {msg.pattern}"
    )
...
def test_budget_fires_on_low_health() -> None:
    ...
    # Reviewer-tightened: 5% remaining must produce a guidance message.
    # Either budget or cost_spiral can win priority.
    assert msg is not None, (
        "budget_health=0.05 produced no GuidanceMessage — at minimum "
        "budget detector should fire under such low health"
    )
